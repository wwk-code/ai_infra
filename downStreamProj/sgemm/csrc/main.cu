...
int main(int argc, char **argv) {
    ...
    if(kernel_seq != 0) {
        test_kernel(0,m,k,n,alpha,A,B,beta,C_ref,cublas_handle);  // CPU MM
        test_kernel(kernel_seq,m,k,n,alpha,dA,dB,beta,dC,cublas_handle);
        CUDA_CALLER(cudaDeviceSynchronize());
        CUDA_CALLER(cudaMemcpy(C,dC,sizeof(DTYPE) * m * n,cudaMemcpyDeviceToHost));
        CUDA_CALLER(cudaDeviceSynchronize());
        if(!verify_cmp_matrix(C,C_ref,m , n)) {
    ...
    if(kernel_seq == 0) {   // custom-cublas-kernel
        ...
        fflush(stdout);
        verify_cmp_matrix(C_ref,C,m , n); //sync C with cuBLAS to prepare for the next run，avoid of diff cumulation error
